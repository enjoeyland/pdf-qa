...
class EmbeddingModel:
    def __init__(self, cfg: EmbeddingConfig):
        ...
        self.model = AutoModel.from_pretrained(cfg.model_id, cache_dir=cfg.cache_dir, **cfg)
...
class LLM:
    def __init__(self, cfg: LLMConfig):
        self.cfg = cfg
        if cfg.backend == "hf":
            self.tok = AutoTokenizer.from_pretrained(cfg.model_id, cache_dir=cfg.cache_dir)
            self.mdl = AutoModelForCausalLM.from_pretrained(cfg.model_id, cache_dir=cfg.cache_dir, **cfg)

    def generate(self, prompt: str) -> str:
        if self.cfg.backend == "hf":
            inputs = self.tok(prompt, return_tensors="pt").to(self.mdl.device)
            with torch.no_grad():
                out = self.mdl.generate(**inputs, max_new_tokens=self.cfg.max_new_tokens, temperature=self.cfg.temperature, pad_token_id=self.tok.eos_token_id)
