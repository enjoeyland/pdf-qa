...
def chunk_with_llm(page_blocks, llm_tokenizer, llm_model):
...
# === 전체 파이프라인 실행 ===
def build_pdf_faiss_index(pdf_path, embed_tokenizer, embed_model):
    all_chunks = []
    ...
    for blocks in page_blocks:
        ...
        for chunk in chunks:
            all_chunks.append({"text": chunk})  # 위치정보를 넣고 싶다면 연결 필요, 이미지 정보도 연결 할 수 있으면 해보기
    ...
    with open(VEC_STORE_DIR, "+a") as f:
        f.write(f"{{'time': {time.time()}, 'source_pdf': '{pdf_path}', 'num_chunks': {len(all_chunks)}, , 'chunks': {all_chunks}}}\n")
...
if __name__ == "__main__":
    ...
    build_pdf_faiss_index(PDF_PATH, embed_tokenizer, embed_model)
